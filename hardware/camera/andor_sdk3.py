...
# Python wrapper installed of the Andor SDK3
# See C:/Program Files/Andor SDK3/Python/pyAndorSDK3/README.pdf for installation help
from pyAndorSDK3 import AndorSDK3


class Main(Base, ScienceCameraInterface):
    """ Hardware class for Andor scientific cameras interfaced with SDK3
    ...
    andor_camera:
        module.Class: 'camera.andor_sdk3.Main'
        default_trigger_mode: 'INTERNAL'
        shutter_TTL: 1
        shutter_switching_time: 100e-3
    """

    _camera_number = ConfigOption('camera_number', 0)
    ...
    def on_activate(self):
        ...
        try:
            ...
            self.log.error('Error during instantiation of the andor SDK.')

        try:
            self._cam = self._sdk.GetCamera(self._camera_number)
            self.log.debug('Connected to camera {}.'.format(self._cam.SerialNumber))
        except:
            self.log.error('Could not open connection to the camera. Is it connected ?'
                           'Is it already used by another software ?')
    ...
    def on_deactivate(self):
        ...
        try:
            ...
            self.log.warning('Error while closing Andor camera.')
    ...
    def _build_constraints(self):
        ...
        constraints.name = cam.CameraName
        constraints.width = cam.PixelHeight
        constraints.height = cam.SensorHeight
        constraints.pixel_size_width = cam.PixelWidth * 1e-6
        constraints.pixel_size_height = cam.PixelHeight * 1e-6
        constraints.internal_gains = cam.options_SimplePreAmpGainControl
        constraints.readout_speeds = cam.options_PixelReadoutRate
        constraints.trigger_modes = cam.options_TriggerMode
        constraints.has_shutter = cam.options_ShutterMode

        constraints.read_modes = []
        if constraints.height > 1:
            constraints.read_modes.append(ReadMode.IMAGE)
        try:
            _ = cam.MultitrackCount # try reading it, otherwise
            constraints.read_modes.append(ReadMode.MULTIPLE_TRACKS)
        except AttributeError:
            pass
        if cam.min_AOIHeight < cam.SensorHeight:  # if option is possible, why not ?
            constraints.read_modes.append(ReadMode.IMAGE_ADVANCED)

        constraints.has_cooler = True  # All Andor camera have one
        temperature_options = np.array(cam.options_TemperatureControl).astype(float)
        constraints.temperature.min = temperature_options.min()
        constraints.temperature.max = temperature_options.max()
        constraints.temperature.step = 1  # Todo: sdk3 use list of options
    ...
    def _wait_for_acquisition(self):
        ...
        self._cam.wait_buffer(1000 * (self._cam.ExposureTime+1))
    ...
    def get_ready_state(self):
        ...
        """
        return not self._cam.CameraAcquiring
